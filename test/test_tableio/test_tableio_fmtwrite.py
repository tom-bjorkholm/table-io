...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param([FmtListRow(values=[], fmt=Fmt())], None,
                     'First row is empty', id='empty-first-row'),
        pytest.param([FmtListRow(values=('only',), fmt=Fmt())], None,
        ...
        pytest.param(
            ...
            None, 'All rows must have the same number of columns', id='ragged'
        ),
        pytest.param(
            [
                FmtListRow(values=('left', 1), fmt=Fmt()),
            ...
            Box(0, 0, 1, None), 'Wrong number of rows', id='too-many-rows'),
        pytest.param(
            [
                FmtListRow(values=('left', 1), fmt=Fmt()),
                FmtListRow(values=('right', 2), fmt=Fmt())
            ],
            Box(0, 0, None, 1), 'Wrong number of columns',
            id='too-many-columns')
    ])
...
def test_write_table_fmtdictdata_normalizes_and_delegates(
    ...
    data = [
        FmtDictRow(values={'alpha': 'left', 'extra': 1}, fmt=Fmt(bold=True)),
    ...
    assert table_io.last_fmtdict_write_data == [
        FmtDictRow(values={'alpha': 'left', 'beta': None}, fmt=Fmt(bold=True)),
        FmtDictRow(values={'alpha': None, 'beta': 2.5}, fmt=Fmt(italic=True))
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param([FmtDictRow(values={}, fmt=Fmt())], ['alpha'], None,
                     'index 0', id='empty-first-row'),
        pytest.param([FmtDictRow(values={'alpha': 'only'}, fmt=Fmt())],
                     ['alpha'], None, 'Data is not at least 2 cells in size',
                     id='one-cell'),
        pytest.param([FmtDictRow(values={'alpha': 'left'}, fmt=Fmt())], [],
                     None, 'column_order', id='empty-column-order'),
        pytest.param([FmtDictRow(values={'alpha': 'left'}, fmt=Fmt())],
                     ['alpha', 'alpha'], None, 'Duplicate column name',
                     id='duplicate-column-order'),
        pytest.param([FmtDictRow(values={'alpha': 'left'}, fmt=Fmt())],
                     ['alpha', 'beta'], None, 'beta', id='missing-column'),
        pytest.param(
            [FmtDictRow(values={'alpha': 'left', 'beta': 1}, fmt=Fmt())],
            ['alpha'], None, 'beta', id='extra-column'),
        pytest.param(
            [FmtDictRow(values={'alpha': 'left', 'beta': 1}, fmt=Fmt())],
            ['alpha', 'beta'], Box(0, 0, 1, None), 'Wrong number of rows',
            id='too-many-rows'),
        pytest.param(
            [FmtDictRow(values={'alpha': 'left', 'beta': 1}, fmt=Fmt())],
            ['alpha', 'beta'], Box(0, 0, None, 1), 'Wrong number of columns',
            id='too-many-columns')
    ])
...
def test_write_table_fmtdictdata_passes_filtered_data_range_when_supported(
    ...
    table_io = WriteSupportedFilteredDataRangeTableIO('sample')
    data = [
        FmtDictRow(values={'alpha': 'left', 'beta': 1}, fmt=Fmt(bold=True))
    ]
    position = table_io.write_table_fmtdictdata(data, ['alpha', 'beta'],
...
def test_write_table_fmtdictdata_rejects_box_when_supported_is_strict(
    ...
    with pytest.raises(CapabilityNotSupported) as exc_info:
        table_io.write_table_fmtdictdata(data, ['alpha', 'beta'],
                                         box=Box(1, 1, 3, 3))
