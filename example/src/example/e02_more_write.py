...
from tableio.value_type import Fmt, ValueFmt, \
    DictData, ListData, FmtListData, FmtDictData, Color, \
...
USE_IF_SUPPORTED = SingleCapability(supported=True, strictness=Strictness.IGNORE)
    ...
    with create_tableio(format_name=format_name,
        ...
        data2: DictData[ValueFmt] = [
            ...
            {'Jira key': ValueFmt(value='TIO-456', fmt=Fmt()),
             ...
             'Assgnee': ValueFmt(value='John Doe', fmt=Fmt(italic=True,
                                                           bold=True,
                                                           highlight=Color.GREEN))},
            {'Jira key': ValueFmt(value='TIO-789', fmt=Fmt()),
             'Reporter': ValueFmt(value='John Doe', fmt=Fmt()),
             'Assgnee': ValueFmt(value='Unassigned', fmt=Fmt(italic=True,
                                                             bold=True,
                                                             highlight=Color.RED))}]
