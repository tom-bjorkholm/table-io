...
@pytest.mark.parametrize('fmt,impl',
                         [('ods', 'odfdo'),
                          ...
                          ('excel', 'XlsxWriter'),
                          ('excel', 'pylightxl')])
def test_e00_write_table(capsys: pytest.CaptureFixture[str], fmt: str,
                         impl: str) -> None:
...
def test_e00_write_table_md_csv(capsys: pytest.CaptureFixture[str], fmt: str,
                                exp: list[str]) -> None:
    """Test 00_really_simple_write_table example for markdown and CSV."""
    example = Example(example_function=e00_really_simple_write_table,
                      format_name=fmt, implementation_name=None)
    check_example_md_csv(example, capture=capsys, expected_fragments=exp)
